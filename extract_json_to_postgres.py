...
def create_postgres_table():
    ...
    """
    try:
        cur.execute(f"""CREATE TABLE IF NOT EXISTS {table_id} (incident_report_number BIGINT PRIMARY KEY, 
...
def write_to_postgres(destination_path: str):
    ...
    df = df_aux.reindex (columns = ['incident_report_number', 'address', 'census_tract', 'clearance_date', 
                                    'clearance_status', 'council_district', 'category_description', 'district', 
                                    'location_type', 'crime_type', 'family_violence', 'occ_date_time', 
    ...
    for _, row in df.iterrows():
        ...
        if result[0] == 0:
            ...
            cur.execute(f"""INSERT INTO {table_id} (incident_report_number, address, census_tract, clearance_date, clearance_status, council_district, category_description, district, location_type, crime_type, family_violence, occ_date_time, x_coordinate, y_coordinate, year, zipcode) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""", (int(row[0]), str(row[1]), float(row[2]), str(row[3]), str(row[4]), (row[5]), str(row[6]), str(row[7]), str(row[8]), str(row[9]), str(row[10]), str(row[11]), float(row[12]), float(row[13]), float(row[14]), str(row[15])))
